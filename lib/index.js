...
module.exports = function (options) {
  ...
  // bundle dirs
  var bundleDirs = json.locals.map(function(bundle) {
    return path.join(options.build, bundle);
  });
  ...
  return function* koaBundler(next) {
    ...
    if (!(yield cofs.exists(target)) || options.force) {
      ...
      // ensure target build/bundle dirs exist, meh!
      yield bundleDirs.map(function(bundleDir) {
        return function(done) { mkdirp(bundleDir, done); }
      });
